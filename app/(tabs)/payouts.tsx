...
import { calculateBasePay, calculateOvertimePay, getBasePayPerHour, getOvertimeMultiplier } from "@/utils/payCalculations";
...
export default function PayoutsScreen() {
  ...
  // Calculate earnings using centralized calculation utility
  const baseHours = 180; // Regular hours worked in the month
  const overtimeHours = 20; // Overtime hours worked in the month
  const basePay = calculateBasePay(baseHours);
  const overtime = calculateOvertimePay(overtimeHours);
  
  // Calculate gross pay from earnings breakdown
  ...
  const grossPay = Math.round(
    earningsBreakdown.basePay +
    earningsBreakdown.overtime +
    earningsBreakdown.performance +
    earningsBreakdown.attendance +
    earningsBreakdown.accuracy +
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <View style={styles.loadingOverlay}>
          <View style={styles.loadingContainer}>
            <ActivityIndicator size="large" color="#5B4EFF" />
            <Text style={styles.loadingText}>Downloading payout slip...</Text>
...
const styles = StyleSheet.create({
  ...
  amountInputContainer: {
    ...
    ...(Platform.OS === 'web'
      ? { outlineWidth: 0, outlineStyle: 'none' as const } // Remove default web outline (long-form)
      : {}
    ),
  },
  amountInputContainerFocused: {
    ...
    ...(Platform.OS === 'web'
      ? {
          boxShadow: '0 0 0 3px rgba(139, 92, 246, 0.1)', // Web focus ring
          outlineWidth: 0,
